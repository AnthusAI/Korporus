...
class TestE2BSDK(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            # New SDK expects Sandbox.create(template=...) instead of ctor args
            self.sandbox = Sandbox.create(template=self.template_id, timeout=3600)
    ...
    def test_sandbox_metadata(self):
        ...
        try:
            # Create a sandbox with custom metadata
            metadata_sandbox = Sandbox.create(template=self.template_id, metadata=test_metadata)
    ...
    def test_sandbox_listing(self):
        ...
        try:
            ...
            list_test_sandbox = Sandbox.create(template=self.template_id, metadata=test_metadata)
            ...
            try:
                ...
                try:
                    ...
                    while paginator.has_next:
                        running_sandboxes.extend(paginator.next_items())
    ...
    def test_connect_to_sandbox(self):
        ...
        new_sandbox = Sandbox.create(template=self.template_id)
        print(f"New sandbox created with ID: {new_sandbox.sandbox_id}")

        # Pause sandbox before connect (API expects paused sandboxes)
        if hasattr(new_sandbox, "beta_pause"):
            print("Pausing sandbox before connect...")
            new_sandbox.beta_pause()
        ...
        try:
            ...
            if NotFoundException is not None and isinstance(e, NotFoundException):
                self.skipTest(f"Sandbox connect not supported in this deployment: {str(e)}")
            else:
                raise
        finally:
    ...
    def test_connecting_to_S3(self):
        ...
        try:
            ...
            if not aws_access_key or not aws_secret_key or not bucket_name:
                self.skipTest("AWS credentials or bucket name are not set")
            ...
            # Use boto3 instead of s3fs to avoid FUSE requirements
            print("Installing boto3 inside sandbox (if missing)...")
            try:
                self.sandbox.commands.run("python -m pip -q install boto3")
            except Exception as e:
                self.skipTest(f"Failed to install boto3 in sandbox: {str(e)}")

            print("Running S3 read/write test with boto3...")
            test_key = f"e2b_test_file_{int(time.time())}.txt"
            test_content = f"E2B S3 connection test at {time.time()}"
            boto3_cmd = (
                "python - <<'PY'\n"
                "import os, boto3\n"
                "bucket=os.environ['AWS_S3_BUCKET_NAME']\n"
                "key=os.environ['E2B_S3_KEY']\n"
                "content=os.environ['E2B_S3_CONTENT']\n"
                "s3=boto3.client('s3',\n"
                ...
                "assert data == content, f'mismatch: {data} != {content}'\n"
            ...
            result = self.sandbox.commands.run(
                ...
                envs={
                    ...
                    "E2B_S3_KEY": test_key,
                    "E2B_S3_CONTENT": test_content,
                },
            )
            self.assertEqual(result.exit_code, 0, f"Failed S3 boto3 test: {result.stderr}")
